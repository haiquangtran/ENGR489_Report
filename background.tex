...
\todo{Look at this uncommented and see if you want to add advantages etc}
% Memory-based methods have the advantages of being able to
% rapidly incorporate the most up-to-date information and relatively
% accurate prediction, but they suffer from poor scalability for large
% numbers of users because the search for all similar users will be
% much slower when the database becomes large. If the size of the
% training set is N, then the time complexity of the memory-based
% method is O(NM) \cite{zeng2003similarity}. 
