...
The use of different events can influence the recommendations that are provided by the system. For example, using multiple explicit ratings such as Likes and Dislikes could affect the ALS algorithm. To understand how different events affect the recommender system, we implement variations of rating information is used with the ALS algorithm.


\todo{Are these names good enough?}
\todo{Should I State this in a pilot study, and then in the actual experiment compare only DUAL ALS vs Hybrid?}
...
\subsubsection{\todo{Item based approach? (Undecided)}}
