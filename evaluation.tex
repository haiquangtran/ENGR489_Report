...
\begin{enumerate} \label{experiment_objectives}
	...
	\item{Does a hybrid weighted CF approach provide superior recommendations to a standard ALS approach?}
...
\end{itemize}
The following programming languages and tools were used in the experiment:
\begin{itemize}
	\item{The programming language Ruby for the application WOTM}
	\item{PredictionIO 9.4 using HBase for the event server, and running on Spark}
...
\begin{figure}
\centering
\includegraphics[scale=0.4]{recent_images/evaluation_workflow.png}
...
\begin{figure}
\centering
\includegraphics[scale=0.4]{recent_images/top_n.png}
...
\todo{
Find a nice compelling number from section 4.3.2 and claim that as an overall improvement compared to some baseline. You should be touting that in the introduction and conclusion.
}
...
This affects the amount of features that will be used in ALS.
